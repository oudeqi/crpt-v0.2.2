...
apiready = function apiready() {
  ...
  var page = new Vue({
    ...
    methods: {
      handlePickCity: function handlePickCity() {
        var _this = this;
        ...
        Utils$1.UI.setCityPicker({
          success: function success(selected) {
            ...
            _this.pcdString = "".concat(province.name, " ").concat(city.name, " ").concat(district.name); // dom.innerHTML = `<span class="fc_c_city_label selected">${province.name} ${city.name} ${district.name}</span>`
          },
      ...
      handleUploadPic: function handleUploadPic() {
        ...
        Utils$1.File.actionSheet('请选择', ['相机', '相册'], function (index) {
          Utils$1.File.getPicture(_this2.uploadImgType[index], function (res, err) {
            if (res) {
              if (res.data) {
                _this2.businessLicense = res.data;
        ...
        return asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee() {
          return regenerator.wrap(function _callee$(_context) {
            while (1) {
              switch (_context.prev = _context.next) {
                case 0:
                  if (!(!_this3.address || !_this3.hasSelected)) {
                    _context.next = 3;
                    break;
                  }

                  Utils$1.UI.toast('请填写企业经营地址');
                  return _context.abrupt("return");

                case 3:
                  if (_this3.businessLicense) {
                    _context.next = 6;
                    break;
                  }

                  Utils$1.UI.toast('请上传营业执照');
                  return _context.abrupt("return");
