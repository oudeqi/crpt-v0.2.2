...
var Service = /*#__PURE__*/function () {
  ...
  createClass(Service, [{
    ...
    value: function queryProductById(productId) {
      return http.get('/crpt-guarantee/gt/product/info/query?productId=' + productId);
    }
  }]);
...
var HeaderController = /*#__PURE__*/function (_Service) {
  ...
  createClass(HeaderController, [{
    ...
    key: "_getDanbao",
    value: function () {
      var _getDanbao2 = asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee() {
        var res;
      ...
      function _getDanbao() {
        return _getDanbao2.apply(this, arguments);
      }
  ...
  createClass(Validation, [{
    ...
    value: function _validate(error) {
      ...
      for (var _i = 0, _Object$keys = Object.keys(state); _i < _Object$keys.length; _i++) {
        ...
        for (var _i2 = 0, _Object$keys2 = Object.keys(state[key].valid || {}); _i2 < _Object$keys2.length; _i2++) {
          k = _Object$keys2[_i2];
          ...
          if (k === 'checked') {
            ...
            if (!valid) {
              error(state[key].valid.checked);
              return valid;
            }
          }

          if (k === 'required') {
            valid = String(state[key].get());

            if (!valid) {
              error(state[key].valid.required);
              return valid;
            }
          }

          if (k === 'max') {
            console.log(parseFloat(state[key].get()));
            console.log(state[key].valid.max);
            valid = parseFloat(state[key].get()) <= state[key].valid.max[0];

            if (!valid) {
              error(state[key].valid.max[1]);
              return valid;
            }
          }

          if (k === 'min') {
            valid = parseFloat(state[key].get()) >= state[key].valid.min[0];

            if (!valid) {
              error(state[key].valid.min[1]);
              return valid;
            }
          }
        }
    ...
    value: function validate(_ref) {
      ...
      if (this._validate(error)) {
        ...
        for (var _i3 = 0, _Object$keys3 = Object.keys(this.state); _i3 < _Object$keys3.length; _i3++) {
          key = _Object$keys3[_i3];

          if (!this.state[key].noRevert) {
            res[key] = this.state[key].get();
          }
        }
...
var PageController = /*#__PURE__*/function (_HeaderController) {
  ...
  function PageController() {
    ...
    /*
    buildType	int	Y	场地变化类型 DIC
    expectInveste	BigDecimal	N	预计投入单位万元（后端转换为元）
    demandMoney	BigDecimal	Y	资金需求单位万元（后端转换为元）
    productId	int	Y	担保产品id
    rate	BigDecimal	Y	产品综合费率
    timeLimit	int	Y	用款期限 单位月
    gtDescribe	int	N	申请说明
     vale	name
    1	新建
    2	改建
    3	扩建
  ...
  createClass(PageController, [{
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                _context.prev = 0;
                _context.next = 3;
                return this.queryProductById(this.state.productId);
    ...
    value: function _initValidation() {
      var validation = new Validation({
        ...
        expectInveste: {
          valid: {
            required: 'expectInveste不能为空',
            max: [100, 'expectInveste不能超过100'],
            min: [10, 'expectInveste不能小于10']
          },
          get: function get() {
        ...
        timeLimit: {
          valid: {
            required: 'timeLimit不能为空',
            max: [100, 'timeLimit不能超过100'],
            min: [10, 'timeLimit不能小于10']
          },
          get: function get() {
    ...
    key: "save",
    value: function () {
      var _save = asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee2() {
        var validation;
        return regenerator.wrap(function _callee2$(_context2) {
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                // let postData = {
                //   buildType: '', // 场地变化类型, 必选  设备贷产品需要，其他隐藏
                //   expectInveste: '', // 选填，客户录入（4位数），设备贷产品需要，其他隐藏,预计投入单位万元（后端转换为元）
                //   demandMoney: '', // 资金需求 单位万元（后端转换为元）默认带出其他系统传入的合同约定金额，可修改小额度
                //   productId: '', // 担保产品id 默认带出推荐的产品，只读
                //   rate: '', // 产品综合费率 默认跟随选择产品带出，只读
                //   timeLimit: '', // 用款期限 单位月 默认带出其他系统传入的合同约定期限，可修改小期限
                // }
                // const checkedRadio = document.querySelector('[name="buildType"]:checked')
                // if (checkedRadio) {
                //   postData.buildType = checkedRadio.value
                // }
                // postData.expectInveste = $api.byId('expectInveste').value
                // postData.demandMoney = $api.byId('demandMoney').value
                // postData.productId = $api.byId('product').value
                // postData.rate = $api.byId('rate').value
                // postData.timeLimit = $api.byId('timeLimit').value
                // console.log(JSON.stringify(postData))
                // if (!postData.buildType) {
                //   return api.toast({ msg: '请选择场地变化类型', location: 'middle' })
                // }
                // if (!postData.demandMoney) {
                //   return api.toast({ msg: '请输入资金需求', location: 'middle' })
                // }
                // if (!postData.rate) {
                //   return api.toast({ msg: '未获取到产品信息', location: 'middle' })
                // }
                // if (!postData.timeLimit) {
                //   return api.toast({ msg: '请输入用款期限', location: 'middle' })
                // }
                validation = this._initValidation();
                validation.validate({
                  ...
                  valid: function valid(data) {
                    console.log(JSON.stringify(data)); // try {
                    //   const res = await this.saveApply(data)
                    //   console.log(JSON.stringify(res))
                    // } catch (e) {
