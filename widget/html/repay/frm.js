...
apiready = function apiready() {
  ...
  function appendTotal(data) {
    $api.byId('total').innerHTML = "\n      <p>\u5269\u4F59\u5F85\u8FD8(\u5143)</p>\n      <p><strong>".concat(numeral(data.remainderRepayAmount).format('0,0.00'), "</strong></p>\n      ").concat(data.remainderRepayAmount ? "<p>\u6700\u8FD1\u8FD8\u6B3E\u65E5\u671F\uFF1A<span>".concat(data.latestRepayDate || '无', "</span> ").concat(data.yuqi ? '<span class="warning">未按期还款</span>' : '', "</p>") : '', "\n      <p>\u5168\u90E8\u5F85\u8FD8\uFF1A<span>").concat(numeral(data.repayTotalAmount).format('0,0.00'), "\u5143</span></p>\n    ");
  }

  function getPageData() {
    $api.byId('total').innerHTML = '<div class="loading">加载中...</div>';
    http.get("/crpt-credit/credit/repay/pending/list?queryDate=".concat(now.format('YYYYMM'))).then(function (res) {
      loading = false;
      api.refreshHeaderLoadDone();
      var data = res.data || {};
      appendList(data.list || []);
      var yuqi = find_1(data.list, function (item) {
        return item.status === 4;
      });
      appendTotal(_objectSpread$1({}, data, {
        yuqi: yuqi
      }));
