...
var service = {
  getChangeList: function getChangeList(params) {
    return http$1.get("/crpt-credit/credit/amount/changeList", {
      values: params
    });
  }
};
  ...
  var page = new Vue({
    ...
    methods: {
            ...
            while (1) {
              switch (_context.prev = _context.next) {
                ...
                case 4:
                  ...
                  if (res.code === 200) {
                    len = res.data.length;
                    _this.list = res.data.map(function (item, index) {
                      var d = new Date(item.changeDate.split(' ')[0]);
                      var isNotFirstItem = index > 0;
                      var prevD = isNotFirstItem ? new Date(res.data[index - 1].changeDate.split(' ')[0]) : ''; // 只有一条数据
      ...
      getNowFormatDate: function getNowFormatDate(date) {
        ...
        var currentdate = month + seperator1 + strDate; // let currentdate = year + seperator1 + month + seperator1 + strDate;
